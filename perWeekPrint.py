...
from connectMysql.connect import dfInit,xzdfyh,pt,tjPt,payMethod
import time
import numpy as np
...
def printT(date,bw,jd,wy):
    ...
    sumseller=[[bwsellerMoney[i][0],round(bwsellerMoney[i][1]+jdsellerMoney[i][1]+wysellerMoney[i][1],2)] for i in range(7)]
    plt.bar([i[0] for i in sumrate],[i[1] for i in sumrate],label='平台管理费')
...
def printXzyh(times,d):
    ...
    for i in times:
        s=xzdfyh(i[0],i[1])
        nums.append([i[2],s])
    plt.bar([i[0] for i in nums],[i[1] for i in nums],alpha=0.9, width=0.6)
    for i in nums:
        plt.text(i[0],i[1]+0.5,i[1],fontsize=8, ha='center', va='center')
    plt.ylabel('注册人数')
    plt.xlabel('日期')
    plt.title(d + '百望商城每天新人注册情况，总注册人数：'+str(sum([i[1] for i in nums])))
    plt.rcParams['font.sans-serif'] = ['SimHei']
    plt.xticks([1, 2, 3, 4, 5, 6, 7], ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'])
...
if __name__ == '__main__':
    ...
    timess = times(starttime)
    endtime=timess[-1][1]
    df=tjPt(starttime,endtime)
    ...
    dfpay=dfInit(starttime,endtime,5)
    payPrint(date,dfpay)
